...
w1 = tf.Variable(tf.random.truncated_normal([18,2], stddev=0.1),dtype=tf.float32) # 四行三列,方差为0.1
b1 = tf.Variable(tf.random.truncated_normal([2], stddev=0.1),dtype=tf.float32) # 一行三列,方差为0.1
...
for epoch in range(epoch):  # 数据集级别的循环，每个epoch循环一次数据集
    for step, (x_train, y_train) in enumerate(train_db):  # batch级别的循环 ，每个step循环一个batch
        with tf.GradientTape() as tape:  # with结构记录梯度信息
            ...
            y_ = tf.one_hot(y_train, depth=2)  # 将标签值转换为独热码格式，方便计算loss
