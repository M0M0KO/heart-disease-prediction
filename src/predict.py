...
data = pd.read_csv(os.path.join(script_dir, "NewData.csv"), header=None)
...
model=tf.keras.models.load_model(os.path.join(script_dir, 'mymodel.h5'), compile=False)
...
np.savetxt(os.path.join(script_dir, "PredictResult.csv"), res, delimiter=",")
print(res)